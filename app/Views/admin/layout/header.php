...
<head>
    ...
    <script>
        var CI_FLASH_MESSAGES = {
            success: <?= json_encode(session()->getFlashdata('message') ?? session()->getFlashdata('success')) ?>,
...
<!-- NAVBAR ADMIN -->
<nav class="fixed top-0 z-50 w-full bg-yellow-bright-light border-b border-yellow-bright-light">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center justify-start rtl:justify-end">
        ...
        <a href="/admin" class="flex ms-2 md:me-24">
          <img src="<?= base_url('assets/ticketly-logo.png') ?>" class="h-14 me-3" alt="Ticketly Logo" />
          <hspan class="hidden md:flex self-center text-xl font-bold sm:text-2xl whitespace-nowrap text-gray-600 ">Admin Panel Center</span>
        </a>
      </div>
...
<!-- SIDEBAR ADMIN -->
<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-24 transition-transform -translate-x-full bg-yellow-bright-light border-r border-yellow-bright-light sm:translate-x-0" aria-label="Sidebar">
   <div class="h-full px-3 pb-4 overflow-y-auto bg-yellow-bright-light">
      <ul class="space-y-2 font-medium">
         <li>
            <?php $uri = service('uri'); ?>
            <a href="/admin" class="flex items-center p-2 rounded-lg group <?= $uri->getSegment(2) == '' ? 'bg-yellow-accent-normal text-black' : 'text-gray-900 hover:bg-yellow-accent-light-hover' ?>">
               <svg class="w-5 h-5 text-gray-700" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 21"><path d="M16.975 11H10V4.025a1 1 0 0 0-1.066-.998 8.5 8.5 0 1 0 9.039 9.039.999.999 0 0 0-1-1.066h.002Z"/><path d="M12.5 0c-.157 0-.311.01-.565.027A1 1 0 0 0 11 1.02V10h8.975a1 1 0 0 0 1-.935c.013-.188.028-.374.028-.565A8.51 8.51 0 0 0 12.5 0Z"/></svg>
               <span class="ms-3">Dashboard</span>
            </a>
         </li>
         <li>
            <a href="/admin/events" class="flex items-center p-2 rounded-lg group <?= $uri->getSegment(2) == 'events' ? 'bg-yellow-accent-normal text-black' : 'text-gray-900 hover:bg-yellow-accent-light-hover' ?>">
               <svg class="w-5 h-5 text-gray-700" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 18 18"><path d="M17 0h-5.768a1 1 0 0 0-.9.58L8.382 5H5.4a1 1 0 0 0-.9.58L1.58 11H0v7h18v-7h-1.58l-2.92-5.42A1 1 0 0 0 12.568 5h-2.982L11.5 1.58A1 1 0 0 0 10.6 1H9.4a1 1 0 0 0-.9.58L6.5 5H3.568a1 1 0 0 0-.9.58L0 11v5h2v-3h14v3h2v-5l-2.618-5.42a1 1 0 0 0-.9-.58H13.4l-2-3.42A1 1 0 0 0 10.6 1H9.4Z"/></svg>
